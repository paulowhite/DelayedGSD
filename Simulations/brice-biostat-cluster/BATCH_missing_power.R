...
if(length(args)>0){
    ...
    ## do
    ## eval 'R CMD BATCH --vanilla "--args iter_sim='$ITER' n.iter_sim=100" BATCH_missing_power.R output/missing_power/missing_power-'$ITER'.Rout &'
    ## done
}else{ ## SLUMR
...
## ** Path
path.res <- file.path("Results","missing-power")
if(dir.exists(path.res)==FALSE){
    dir.create(path.res)
}
path.output <- file.path("output","missing-power")
...
## * Settings
name <- "Missing_Power_2_analyses" # To save the results
