...
if(length(args)>0){
    ...
    ## do
    ## eval 'R CMD BATCH --vanilla "--args iter_sim='$ITER' n.iter_sim=100" BATCH_nomissing_typeI.R output/nomissing_typeI/nomissing_typeI-'$ITER'.Rout &'
    ## done
}else{ ## SLUMR
...
## ** Path
path.res <- file.path("Results","nomissing_typeI")
if(dir.exists(path.res)==FALSE){
    dir.create(path.res)
}
path.output <- file.path("output","nomissing_typeI")
