...
if(length(args)>0){
    ...
    ## do
    ## eval 'R CMD BATCH --vanilla "--args iter_sim='$ITER' n.iter_sim=100" BATCH_missing_typeI.R output/missing_typeI/missing_typeI-'$ITER'.Rout &'
    ## done
}else{ ## SLUMR
    iter_sim <- as.numeric(Sys.getenv("SLURM_ARRAY_TASK_ID"))
    n.iter_sim <- as.numeric(Sys.getenv("SLURM_ARRAY_TASK_COUNT"))
} ## interactive
if(is.na(iter_sim)){iter_sim <- 100}
if(is.na(n.iter_sim)){n.iter_sim <- 100}
cat("BATCH ",iter_sim," over ",n.iter_sim,"\n",sep="")

## ** Path
path.res <- file.path("Results","missing_typeI")
if(dir.exists(path.res)==FALSE){
    dir.create(path.res)
}
path.output <- file.path("output","missing_typeI")
