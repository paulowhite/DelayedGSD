...
if(length(args)>0){
    ...
    ## do
    ## eval 'R CMD BATCH --vanilla "--args iter_sim='$ITER' n.iter_sim=100" BATCH_noFixC_power.R output/noFixC_power/noFixC_power-'$ITER'.Rout &'
    ## done
}else{ ## SLUMR
...
## ** Path
path.res <- file.path("Results","noFixC_power")
if(dir.exists(path.res)==FALSE){
    dir.create(path.res)
}
path.output <- file.path("output","noFixC_power")
