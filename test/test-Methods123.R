...
## Version: 
## Last-Updated: feb 17 2022 (16:34) 
##           By: Brice Ozenne
##     Update #: 2
##----------------------------------------------------------------------
## 
...
test_that("Boundary calculation with Method 1",{
    ...
    b12 <- Method1(Kmax=2,delta=1.5,alpha=0.025,beta=0.2,InfoR.i=c(0.6,1),InfoR.d=0.65,binding=T)
   
  expect_equal(b1$lower$bound, b12$boundaries[,"l.k"], tol = 1e-4)
  expect_equal(b1$upper$bound, b12$boundaries[,"u.k"], tol = 1e-4)
  ...
  b12nb <- Method1(Kmax=2,delta=1.5,alpha=0.025,beta=0.2,InfoR.i=c(0.6,1),InfoR.d=0.65,binding=F)
  
  expect_equal(b1nb$lower$bound, b12nb$boundaries[,"l.k"], tol = 1e-4)
  expect_equal(b1nb$upper$bound, b12nb$boundaries[,"u.k"], tol = 1e-4)
  ...
  expect_equal(b2$lower$bound, b22$boundaries[,"l.k"], tol = 1e-4)
  expect_equal(b2$upper$bound, b22$boundaries[,"u.k"], tol = 1e-4)
